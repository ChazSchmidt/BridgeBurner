...
contract DateMaker {
    uint uint256 IDcounter;  // used for dateID and corresponding streamID
    
}



contract Date is Streams{
    ...
    constructor(address _invited) { // should pass all terms to create stream
        
    host = msg.sender;
    invited = _invited;
        
    stream = // collect stream terms to be verified
    }
...
contract Streams is Ownable {
  ...
  struct Timeframe {
    uint256 start;
    uint256 stop;pragma solidity >=0.5.1 <0.6.0; 
//import "./ownable.sol";
//import "./Streams.sol";
...
contract Date {
    ...
    constructor(address _invited) { // should pass all terms to later create stream
